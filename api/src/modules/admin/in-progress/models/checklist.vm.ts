...
export async function makeChecklistViewModel(checklistEntity: OrderCheckEntity): Promise<ChecklistViewModel> {
  ...
  await Promise.all(attachmentsEntities.map(
    attachment => attachment.photo.then(photo => {
      if (photo) {
  ...
  const actionsViewModelsPromises: Array<Promise<ActionChecklistViewModel>> = actionsEntities.map(action => makeActionChecklistViewModel(action));
...
export async function makeActionChecklistViewModel(actionChecklist: OrderCheckActionEntity): Promise<ActionChecklistViewModel> {
